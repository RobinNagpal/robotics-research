# Part 03 — Decapping & capping (simulation)

> **Sim goal:** Prove the *logic and choreography* of opening and
> re-closing a vial — present, decap, work, recap, verify — without
> pretending to simulate threaded-cap physics, which we deliberately
> abstract away.
...
[`../02-high-level-solution/03-decapping-and-capping.md`](../02-high-level-solution/03-decapping-and-capping.md)).
...
torque event itself as an abstract, mockable action.

**Can prove in open-source sim:**

- The **arm reaches the station** with the held vial and presents it at
  the correct pose and orientation (vial axis aligned, upright).
...
pierces). These are friction/material effects we abstract; they must be
validated on the real station/tool. Sim still de-risks them by proving
the surrounding choreography is correct *before* you spend the torque
budget on hardware bring-up.
...
| **`gz_ros2_control`** | Drives the reBot URDF joints in sim (no real driver needed) | Same joint interface as the real arm. |
| **MoveIt 2** | Plans the collision-free move to the station presentation pose | Config generated from the reBot URDF. |
...
3. **Expose station tf frames.** Publish a static transform for
   `decap_station` (and a `cap_nest_<vial_id>` parking frame). The arm
...
6. **Verify with perception, not physics.** Don't trust the service's
   word alone — gate on Part 07 reading the rendered scene
   (cap link present vs absent, cap seating height). This mirrors the
   real recap-integrity check.
...
Beyond the reBot arm + its parallel gripper, the real bench needs **one**
of:
...
- a **torque-controlled decapping end-effector** the arm picks up from a
  **tool changer** (quick-connect interface) and uses to decap in place.
