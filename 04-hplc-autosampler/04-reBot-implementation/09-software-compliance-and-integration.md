# Part 09 — Software, compliance & integration (simulation)
...
> cell and a real one, so you can build roughly ~90% of it now with
> **zero hardware**.
...
This whole layer is **pure software**. It never touches a motor or a
camera directly — it ingests a worklist, issues high-level steps to
the orchestration layer (Part 08), and records what happened. The
orchestrator does not care whether the arm underneath it is a real
reBot or the Gazebo twin: it speaks the same ROS 2 interfaces either
way (`FollowJointTrajectory`, `GripperCommand`, the mock-station
services). So **every line of this layer that you write against the
simulated cell carries over unchanged** to the real one.
...
- **CDS — Chromatography Data System.** The software that drives the
  HPLC instrument and holds the **worklist** (the ordered list of
  which vial runs with which method). Dominant products: **Waters
  Empower** and **Thermo Scientific Chromeleon**.
- **21 CFR Part 11.** The US FDA rule that says electronic records
  and signatures must be as trustworthy as wet-ink paper — controlled,
  attributable, and traceable. (EU equivalent: **EU GMP Annex 11**.)
...
- **Driving the Behavior Tree.** The controller issues high-level
  ...
  after replay, against the Gazebo cell.
- **Immutable audit trail.** Every action (who/what/when, and why)
  is written to an **append-only** store — see below. You can run a
  batch and then inspect the exact trail an auditor would read.
...
- **CSV — Computer System Validation** — the documented lifecycle
  that keeps the software in a validated state. This is a funded,
...
- **Real CDS API behaviour** — Empower / Chromeleon toolkits,
  ...
  mock proves the *shape* of the integration, not the vendor's
  actual API.

So sim proves the **software logic, the data-integrity story, and
the integration contract** end to end. It does **not** discharge the
...
| **ROS 2** (Humble/Jazzy) | Carries the controller↔orchestrator messages, identical in sim and on hardware | Same interface whichever arm is underneath |
| **Gazebo Harmonic** | Hosts the simulated cell the controller drives during replay | The "cell" the software talks to before hardware exists |
...
**1. Stand up the mock LIMS/CDS.** A small **FastAPI** service with
two endpoints:

- `GET /worklist` — returns a sequence of rows (sample ID, recipe,
  order, target slot), loaded from a **CSV** file you control. This
  is the stand-in for pulling a sequence from Empower/Chromeleon.
...
**2. Run the controller alongside the Gazebo cell.** The controller:

- Pulls the worklist from the mock CDS.
- For each row, calls the Part 08 orchestrator (Behavior Tree) to
  prep and load that vial in Gazebo.
...
own audit event (who, what record, when, the meaning of the
signature, e.g. "reviewed and approved for injection").
...
- If using the hash-chained log, run the verifier to show the chain
  is intact (tamper-evident).
- Confirm no `/ready-to-inject` hand-off ever fires without a valid
  reviewer e-signature in the trail.

**Workflow per batch:** pull worklist (mock CDS) → controller drives
BT per row against Gazebo → log every action to the append-only
...
|---------------|-----|----------------------|
| **None for development** | This layer is pure software; it runs against the simulated cell exactly as against a real one | The Gazebo twin *is* the cell; mock FastAPI *is* the CDS/LIMS |
...
Because the layer is software, **the whole thing can be built and
demonstrated now** — the only "hardware" it needs is the simulator
standing in for the cell.

## How it connects

- [`06-identification-labeling-and-tracking.md`](06-identification-labeling-and-tracking.md)
  — supplies the sample IDs and the worklist this controller
  ingests, and consumes the as-prepared record; the audit trail ties
  each ID to its actions.
...
- [`10-hardware-platform-and-capital-model.md`](10-hardware-platform-and-capital-model.md)
  — validation and containment requirements here drive hardware and
  layout choices there (a cell you can put a boundary around is a
...
> that picks a vial is a solved-ish problem; a controller an FDA or
> EU inspector will accept — Part 11 audit trail, e-signatures, RBAC,
> a CSV/IQ/OQ/PQ package, clean CDS integration — is months of
> specialised work most robotics teams lack. Sim lets you build and
> prove ~90% of that software *before* spending on hardware, which is
> exactly why this layer, not the arm, is the real competitive moat.
> **Re-verify** the exact Part 11 / Annex 11 / ALCOA+ expectations
> against current FDA/EU guidance and the customer's own quality SOPs
> before quoting anything — interpretation drifts.
