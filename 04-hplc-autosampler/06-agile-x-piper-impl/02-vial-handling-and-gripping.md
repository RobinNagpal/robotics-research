# Part 02 — Vial handling & gripping (AgileX PiPER simulation)

> **Sim goal:** Prove in open-source sim that the AgileX PiPER's
...
A **gripper** here is the small hand on the arm's end. The PiPER ships
with a **parallel gripper** (two jaws that close straight toward each
other). A **2 mL vial** is the small glass cylinder an HPLC
autosampler draws from — roughly ~12 mm across and ~32 mm tall.
Picking something that small and smooth is the most contact-sensitive
motion in the whole cell, so it gets its own part.

A note on the PiPER's envelope up front: its ~1.5 kg payload is *far*
more than the few grams a vial weighs — payload is a complete
non-issue, with enormous headroom for fingertips, fixtures, or a
heavier custom end-effector. Its ~600 mm reach also means the supply
rack can sit at a comfortable, well-conditioned spot rather than at
the ragged edge of the workspace where IK solutions get scarce and
approach angles awkward (the placement squeeze a myCobot 280 imposes).
So on the PiPER, this part is about **grasp geometry and contact
choreography**, not about fighting the arm's limits.
...
- **Reach-aware rack placement.** That the chosen rack location yields
  good IK and clean approach for *every* nest — comfortable on this
  arm, but still worth confirming so poses stay well away from
  singularities.
...
| `piper_ros` gripper model | Gripper links/joint in the PiPER URDF | Reuse the shipped description; command via the gripper action. |
...
`gz_ros2_control` and answers `FollowJointTrajectory`, and the gripper
is driven through a `GripperCommand`-style action.
...
`vial_supply` tf frame from Part 01. Because the PiPER's reach is
generous, the rack can sit at a comfortable, well-conditioned location
rather than at the workspace edge — so each nest (e.g.
`vial_supply/slot_03`) has a known pose with a healthy IK margin.
Known poses let us skip vision for now — perception-driven grasping is
layered in by Part 07; here the slot geometry is given.
...
sized to the diameter). Model the parallel jaw geometry so the fingers
wrap the ~12 mm body, not the cap. Store these as tf offsets so the
same numbers drive every nest.
...
- Send the `GripperCommand` close command sized to the vial diameter.
...
to the gripper link. From that instant the vial moves rigidly with the
hand — no reliance on flaky contact friction. Releasing (open command)
deletes the joint and drops the vial. For contact-detail work (finger
pad deformation, minimum hold force) re-create the same grasp in
**MuJoCo**, which handles small-object contact far more stably.
...
- Plan a transport motion toward a downstream station frame
  ...
  attached vial as part of the arm for collision checking). With the
  PiPER's longer links there is more arm sweeping through the cell, so
  watch for the upper links — not just the held vial — clipping
  station models.
...
- **Gripper** — the PiPER **parallel gripper** (or, if grasping smooth
  glass proves marginal on hardware, a **suction** or custom
  end-effector — the ~1.5 kg payload leaves ample room for a heavier
  one); modelled as the gripper links in the `piper_ros` URDF and
...
- **Compliant elastomer fingertips** — soft fingertip pads that
  ...
  exactly what sim cannot prove).
- **Physical vial supply rack** — modelled as the nested rack model
  with known slot frames, placed at a comfortable spot inside the
  PiPER's reach.
...
- `01-scope-and-workflow.md` — provides the world, the arm spawn,
  MoveIt config, the reachable workspace, and the `vial_supply` frame
  this part populates.
...
- High-level companion:
  `../02-high-level-solution/02-vial-handling-and-gripping.md`.
