# Part 09 — Software, compliance & integration (AgileX PiPER simulation)

> **Sim goal:** Prove that the *defensible* part of the product — the
> controller, the audit trail, the access/e-signature controls, and the
> LIMS/CDS hand-off — runs **identically** against the simulated
> **AgileX PiPER** cell and against any real cell, so you can build
> roughly ~90% of it now with **zero hardware** and with no dependence
> on which arm you eventually buy.

This mirrors the high-level
[`../02-high-level-solution/09-software-compliance-and-integration.md`](../02-high-level-solution/09-software-compliance-and-integration.md).
...
whether the arm underneath is the Gazebo **PiPER** twin, a **myCobot
280/320**, a **reBot**, a **UR3e/UR5e**, or the real PiPER driven by
`piper_sdk` over its CAN bus. So **every line of this layer you write
against the simulated PiPER cell carries over unchanged** — both to the
real arm and to a *different* arm if the platform decision in Part 10
changes. That makes this the safest, highest-value code to build first.
...
- **CDS — Chromatography Data System.** The software that drives the
  **HPLC** (High-Performance Liquid Chromatography) instrument and holds
  the **worklist** — the ordered list of which vial runs with which
  method. Dominant products: **Waters Empower** and **Thermo Scientific
  Chromeleon**.
- **21 CFR Part 11.** The US FDA (Food and Drug Administration) rule
  that says electronic records and signatures must be as trustworthy as
  wet-ink paper — controlled, attributable, and traceable. (EU
...
- **Worklist ingestion.** The controller reads a worklist — a **CSV**
  ...
  prep-and-load job.
- **Driving the Behavior Tree.** The controller issues high-level steps
  to the Part 08 orchestrator and consumes the results, replay after
  replay, against the Gazebo PiPER cell.
...
- **Electronic review / approval / e-signature** — a reviewer inspects
  the as-prepared record in the UI and signs it before the tray is
  "released," with the signature capturing who/what/when/meaning.
- **Mock LIMS/CDS integration** — a stand-in service supplies the
  worklist and receives the "ready to inject" hand-off, exercising the
...
- **IQ/OQ/PQ qualification** — **I**nstallation / **O**perational /
  **P**erformance **Q**ualification: documented proof the system was
  installed right, operates right, and performs right *in real use*. You
  cannot qualify a simulated installation.
...
- **Real CDS API behaviour** — Empower / Chromeleon toolkits, licensing,
  and quirks must be tested against the real product; our mock proves
  the *shape* of the integration, not the vendor's actual API.
...
regulatory **validation** obligation — that is explicitly deferred (see
the moat note below).
...
| **ROS 2** (Humble/Jazzy) | Carries the controller↔orchestrator messages, identical in sim and on any arm | Same interface whether PiPER, myCobot, reBot, or UR is underneath |
| **Gazebo Harmonic** | Hosts the simulated PiPER cell the controller drives during replay | The "cell" the software talks to before hardware exists |
...
- `POST /ready-to-inject` — receives the controller's as-prepared record
  (which vial went to which slot, with the audit references) when the
  tray is loaded and signed off. This is the hand-off the real CDS would
  consume to start the run.

**2. Run the controller alongside the Gazebo PiPER cell.** The
controller:

- Pulls the worklist from the mock CDS.
- For each row, calls the Part 08 orchestrator (Behavior Tree) to prep
  and load that vial in Gazebo — driving the **PiPER URDF** (from
  `piper_ros`) through `gz_ros2_control` + MoveIt 2 exactly as it would
  drive the real arm through `piper_sdk`.
- Writes an **audit event** for every meaningful action and decision —
  worklist imported, vial picked, decapped, dispensed, recapped, placed
  in slot, verified, any retry or quarantine — each stamped with the
  **user**, a **UTC timestamp**, the **vial/sample ID**, and the
  **outcome**. Write-once: never overwrite, never delete.

**3. Enforce access control + e-signature.** Seed a few named users with
roles (operator runs batches; reviewer approves; admin manages users).
Before the controller calls `/ready-to-inject`, require a **reviewer
e-signature** on the as-prepared record — captured as its own audit
event (who, what record, when, the meaning of the signature, e.g.
"reviewed and approved for injection").
...
- Dump the **audit trail** and read it as an auditor would — confirm
  every action is attributable, contemporaneous, and complete (ALCOA+),
  and that failures and retries are all recorded.
...
per row against the Gazebo PiPER → log every action to the append-only
store → reviewer e-signs the as-prepared record → controller posts
`ready-to-inject` → inspect audit trail + e-sign records.

## Additional hardware needed

| Real hardware | Why | How mocked / handled |
|---------------|-----|----------------------|
| **None for development** | This layer is pure software; it runs against the simulated cell exactly as against a real one | The Gazebo PiPER twin *is* the cell; mock FastAPI *is* the CDS/LIMS |
...
Because the layer is software and arm-agnostic, **the whole thing can be
built and demonstrated now** — the only "hardware" it needs is the
simulator standing in for the cell, and nothing here changes if you
later trade the PiPER for a myCobot 320 or a UR arm.
...
- [`08-orchestration-error-handling-and-safety.md`](08-orchestration-error-handling-and-safety.md)
  — the controller drives this Behavior Tree and **logs every decision,
  retry, and failure** it reports into the audit trail.
- [`10-hardware-platform-and-capital-model.md`](10-hardware-platform-and-capital-model.md)
  — validation and containment requirements here drive the hardware and
  layout choices there (a cell you can put a boundary around is a cell
  you can IQ/OQ/PQ).
- Mirrors
  [`../02-high-level-solution/09-software-compliance-and-integration.md`](../02-high-level-solution/09-software-compliance-and-integration.md);
...
> CSV/IQ/OQ/PQ package, clean CDS integration — is months of specialised
> work most robotics teams lack. Crucially, because it is
...
> PiPER, a myCobot, a reBot, or a UR cell. (The PiPER's LeRobot
> compatibility is a bonus *later* — for imitation-learning skills — but
> it does not change this layer.) Sim lets you build and prove ~90% of
> it *before* spending on hardware, which is exactly why this layer, not
> the arm, is the real competitive moat. **Re-verify** the exact Part 11
> / Annex 11 / ALCOA+ expectations against current FDA/EU guidance and
> the customer's own quality SOPs before quoting anything —
