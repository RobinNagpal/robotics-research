# Part 04 — Liquid handling & sample prep (AgileX PiPER simulation)

> **Sim goal:** Prove the *sample-prep logic* with the **AgileX PiPER**
> arm — which vial gets which volume of which reagent, in what order,
> with washes between — by tracking liquid as a **number**, not by
> simulating any fluid dynamics.
...
[`../02-high-level-solution/04-liquid-handling-and-sample-prep.md`](../02-high-level-solution/04-liquid-handling-and-sample-prep.md).
Computational fluid dynamics (CFD — software that simulates how liquids
...
*reagent* is any liquid being added — diluent, internal standard, and so
on. A dispenser *service* mutates that state.

**Can prove in open-source sim:**

- The **dispense choreography**: grip open vial → move under the nozzle
  frame → call `/dispense` → state updates → move on. The PiPER's
  ~600 mm reach (`~`, verify) lets the dispense nozzle sit at its own
  station, away from the decapper and tray.
...
- The **wash / carryover *policy*** as logic and timing: a wash step is
  ...
  physical cleaning.
- An optional **gravimetric check** (a weigh-based volume check) as
...
| **`gz_ros2_control`** | Drives the **PiPER URDF** joints under the nozzle | No real driver needed; `piper_ros` URDF. |
| **MoveIt 2** | Plans the collision-free move to the dispense pose | Config from the PiPER URDF. |
...
3. **Expose the nozzle tf frame.** Publish a static transform
   `dispense_nozzle`. The PiPER plans a **dispense pose** placing the
   open vial mouth under that frame. (*tf* is the ROS 2 system of named
   coordinate frames that tracks where everything is.)
...
Beyond the **PiPER arm + gripper**, the real bench needs the actual
fluidics:
...
stations the arm presents vials to. Watch the PiPER's ~1.5 kg payload
(`~`, verify) if a fluidics tool is arm-mounted rather than stationary.
