...
> folder (and both `01-only-code/` and `02-code-plus-hardware/`) refers
> back here instead of re-defining the suite.
...
- **Safety + IMU + limits (9–12)** →
  [`08-orchestration-error-handling-and-safety.md`](08-orchestration-error-handling-and-safety.md)
  and the orchestration layer files; all sensor readings are logged for
  the audit trail in
